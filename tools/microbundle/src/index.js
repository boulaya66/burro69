...
function createConfig(options, entry, format, writeMeta) {
	...
	let config = {
		...
		inputOptions: {
			...
			// so that cjs symlinked modules are correctly parsed by commonjs plugin
			...
			plugins: []
				.concat(
					...
					json(),
					{
						...
						transform: code => ({
							code: code.replace(/^#![^\n]*/, bang => {
								// options.name is the same for all entries
								// shebang[options.name] = bang;
								shebang[outputEntryFileName] = bang;
							}),
							map: null,
						}),
					},
		...
		outputOptions: {
			...
			sourcemap: options.sourcemap,
			get banner() {
				return shebang[outputEntryFileName]; // shebang[options.name];
			},
